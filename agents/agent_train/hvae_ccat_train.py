...
from agents.models.hvae_ccat_model import *
from agents.models.language_generator import *
import random
from utils.tools import *


class HVAECCatAgent(nn.Module):
    '''
    NO S_h categorical latent state
    '''
    ...
    def __init__(self, args=None, env=None, config=None, train=True):
        ...
        if not os.path.exists(f'./tmp/{args.env}_{args.model_id}_{args.method}/'):
            os.makedirs(f'./tmp/{args.env}_{args.model_id}_{args.method}/')
        device = args.device
        ...
        self.hvae = HVAECCat(h_size=config.h_size,
        ...
        self.reward_models_optimizer = torch.optim.Adam(get_parameters([self.hvae.r_reward_decode_model,
                                                                        self.hvae.h_reward_decode_model]), lr=config.lr)
        self.pt_model_optimizer = torch.optim.Adam(get_parameters([
            self.hvae.s_embed_decode_model,
            self.hvae.xs_embed_decode_model,
            self.hvae.h_embed_decode_model,
            self.hvae.pt_model]), lr=config.pt_lr)
    ...
    def save_pt_models(self, episode=''):
        ...
        self.hvae.s_embed_decode_model.save_checkpoint(episode)
        self.hvae.xs_embed_decode_model.save_checkpoint(episode)
        self.hvae.h_embed_decode_model.save_checkpoint(episode)
        self.hvae.pt_model.save_checkpoint(episode)

    def load_vae_models(self, episode=''):
        print('.... loading VAE models ....')
        self.hvae.load_checkpoint()
    ...
    def learn_pt(self, learn_itr):
        ...
        self.eval()
        self.hvae.s_embed_decode_model.train()
        self.hvae.xs_embed_decode_model.train()
        self.hvae.h_embed_decode_model.train()
        self.hvae.pt_model.train()
        ...
        for i in tqdm(range(self.max_epoch), desc='training'):
            ...
            # pi_dict = self.hvae.get_default_pi(batch_shape=pos.shape[:-1])
            ...
            loss = pt_loss
            # obs_dict = {'agent_obs': agent_obs_dict,
            ...
            # agent_embed_rec_loss = self.config.agent_embed_rec_scale * torch.square(agent_embed_rec - post_dict['s_embed'].clone().detach()).mean()
            #
            # # train for debugging only
            # agent_rec_loss = 0.0
            # agent_rec_from_embed = {}
            # for obs_name, obs_data in agent_obs_dict.items():
            #     agent_rec_from_embed[obs_name] = self.hvae.xs_embed_decode_model.modality_modules[obs_name](post_dict['s_embed'].clone().detach())
            #
            ...
            #
            # pi_embed_rec = self.hvae.h_embed_decode_model(torch.cat([post_dict['h_sample'].clone().detach(), post_dict['s_sample'].clone().detach()], dim=-1))
            # pi_embed_rec_loss = self.config.pi_embed_rec_scale * torch.square(pi_embed_rec - post_dict['h_embed'].clone().detach()).mean()
            #
            # # perspective prediction
            # perspective_est = self.hvae.pt_model(torch.cat([post_dict['h_embed'].clone().detach(),
            #                                                 post_dict['zp_sample'].clone().detach()], dim=-1)).mean
            # pt_loss = self.config.pt_rec_scale * torch.square(perspective_est - post_dict['s_embed'].clone().detach()).mean()
            #
            # loss = agent_embed_rec_loss + agent_rec_loss + pi_embed_rec_loss + pt_loss

            self.pt_model_optimizer.zero_grad()
            loss.backward()
            self.pt_model_optimizer.step()

            # agent_embed_losses.append(agent_embed_rec_loss.item())
            # pi_embed_losses.append(pi_embed_rec_loss.item())
            pt_losses.append(pt_loss.item())
        print(f'agent_embed_loss:{torch.tensor(agent_embed_losses).mean()}')
        print(f'pi_embed_loss:{torch.tensor(pi_embed_losses).mean()}')
        print(f'pt_loss:{torch.tensor(pt_losses).mean()}')
        self.writer.add_scalar('Training/agent_embed_loss', torch.tensor(agent_embed_losses).mean(), learn_itr)
        self.writer.add_scalar('Training/pi_embed_loss', torch.tensor(pi_embed_losses).mean(), learn_itr)
        self.writer.add_scalar('Training/pt_loss', torch.tensor(pt_losses).mean(), learn_itr)
        ...
        # pt_img = self.hvae.xs_embed_decode_model.modality_modules['img'](perspective_est.clone().detach())
    ...
    def learn_language(self, learn_itr):
        ...
        comm_generator_losses = []
        feedback_generator_losses = []
        comm_decode_losses = []
        query_generator_losses = []

        self.eval()
        self.comm_generator.train()
        self.feedback_generator.train()
        self.query_generator.train()
        self.hvae.comm_decode_model.train()
        ...
        for i in tqdm(range(self.max_epoch), desc='training'):
            ...
            comm_decode_loss = torch.square(self.hvae.comm_decode_model(e_embed)['text'] - self_comm_dict['text']['data']).mean()
            ...
            comm_generator_losses.append(comm_generator_loss.item())
            feedback_generator_losses.append(feedback_generator_loss.item())
            comm_decode_losses.append(comm_decode_loss.item())
            query_generator_losses.append(query_generator_loss.item())

        self.writer.add_scalar('Training/comm_generator_loss', torch.tensor(comm_generator_losses).mean(), learn_itr)
        self.writer.add_scalar('Training/feedback_generator_loss', torch.tensor(feedback_generator_losses).mean(), learn_itr)
        self.writer.add_scalar('Training/comm_decode_loss', torch.tensor(comm_decode_losses).mean(), learn_itr)
        self.writer.add_scalar('Training/query_generator_loss', torch.tensor(query_generator_losses).mean(), learn_itr)
    ...
    def learn_human_policy(self, learn_itr):
        ...
        for i in tqdm(range(self.max_epoch), desc='training'):
            ...
            feat_sac = torch.cat([post_dict['h_mean'][:-1][:-1], post_dict['s_sample'][:-1][:-1]], dim=-1)
            ...
            with torch.no_grad():
                target_next_feat_sac = torch.cat([post_dict['h_mean'][:-1][1:], post_dict['s_sample'][:-1][1:]], dim=-1)
        ...
        self.writer.add_scalar('Training/human_policy_loss', torch.tensor(qf_losses).mean(), learn_itr)

    def learn(self, learn_itr, learn_reward_only=False):
        if self.human_memory.mem_cntr < self.batch_size * self.batch_length + 1:
            return
        ...
        for i in tqdm(range(self.max_epoch), desc='training'):
            r_loss, r_rec_loss, r_pi_loss, r_xp_rec_loss, r_reward_rec_loss, r_pt_loss, r_kl_loss = self.get_loss(
                'robot', random_batch_length, i)
            h_loss, h_rec_loss, h_pi_loss, h_xp_rec_loss, h_reward_rec_loss, h_pt_loss, h_kl_loss = self.get_loss(
                'human', random_batch_length, i)

            if learn_reward_only:
                self.reward_models_optimizer.zero_grad()
                loss = r_reward_rec_loss + h_reward_rec_loss
                loss.backward()
            ...
            xp_rec_losses += [r_xp_rec_loss.item() + h_xp_rec_loss.item()]
            reward_losses += [r_reward_rec_loss.item() + h_reward_rec_loss.item()]
            pt_losses += [r_pt_loss.item() + h_pt_loss.item()]
            kl_losses += [r_kl_loss.item() + h_kl_loss.item()]
        ...
        self.writer.add_scalar('Training/xp_rec_losses', torch.tensor(xp_rec_losses).mean(), learn_itr)
        self.writer.add_scalar('Training/reward_loss', torch.tensor(reward_losses).mean(), learn_itr)
        self.writer.add_scalar('Training/pt_loss', torch.tensor(pt_losses).mean(), learn_itr)
        self.writer.add_scalar('Training/kl_loss', torch.tensor(kl_losses).mean(), learn_itr)
        ...
        print(f'pt_loss:{torch.tensor(pt_losses).mean()}')
        print(f'kl_loss:{torch.tensor(kl_losses).mean()}')
    ...
    def get_loss(self, role, random_batch_length, itr_count):
        ...
        for pi_name, pi_data in pi_dict.items():
            pi_rec[pi_name] = self.hvae.xh_decode_model.modality_modules[pi_name](torch.cat([post_dict['h_sample'], post_dict['s_sample']], dim=-1))
        ...
        xp_rec = self.hvae.zp_decode_model(post_dict['zp_sample'])
        xp_rec_loss = 1.0 * torch.clip(self.config.xp_rec_scale
                                       * torch.square(pos - xp_rec).mean(),
        ...
        if self.reward_mode == 'st_at':
            reward_model_input = torch.cat(
                [post_dict['h_sample'][:-1], post_dict['s_sample'][:-1], action[1:]],
                dim=-1).clone().detach()
            ...
            reward_model_input = torch.cat([post_dict['h_sample'][:-1], post_dict['s_sample'][:-1]], dim=-1).clone().detach()
            reward_dist = reward_decode_model(reward_model_input)
        ...
        pi_rec_dict = {
            modality_name: {
                'type': self.hvae.xh_decode_model.modality_info_dict[modality_name]['modality_type'],
                'data': modality_data.detach(),
                'mask': 1
            }
            for modality_name, modality_data in pi_rec.items()}
        pt_embed = self.hvae.pt_embed_model(pi_rec_dict)
        pt_embed = torch.cat([pt_embed, xp_rec.detach()], dim=-1)
        for obs_name, obs_data in agent_obs_dict.items():
            pt_rec[obs_name] = self.hvae.pt_decode_model.modality_modules[obs_name](pt_embed)

            pt_loss += torch.clip(self.config.agent_rec_scale[obs_name]
                                  * torch.square(pt_rec[obs_name] - obs_data['data']).mean(),
                                  min=self.config.agent_rec_lb[obs_name], max=1000000.0)

        # kl for transition
        # kl_loss = self.config.kldiv_s_scale * kl_divergence_normal(
        #     post_dict['s_mean'], prior_dict['s_mean'], post_dict['s_std'], prior_dict['s_std'])
        # kl_loss += self.config.kldiv_h_scale * kl_divergence_cat(
        #     post_dict['h_mean'], prior_dict['h_mean'])
        # kl_loss += self.config.kldiv_zp_scale * kl_divergence_normal(
        #     post_dict['zp_mean'], prior_dict['zp_mean'], post_dict['zp_std'], prior_dict['zp_std'])

        # loss = agent_rec_loss + pi_rec_loss + xp_rec_loss + reward_rec_loss + done_rec_loss + pt_loss \
        #      + torch.max(kl_loss, torch.ones(1).to(kl_loss.device) * self.anneal_factor)

        # KL Balancing and Free Bits from DreamerV3
        kl_dyn_s_loss = kl_divergence_normal(
            post_dict['s_mean'].detach(), prior_dict['s_mean'],
            post_dict['s_std'].detach(), prior_dict['s_std'])
        kl_dyn_s_loss = torch.max(
            kl_dyn_s_loss, torch.tensor(self.config.kl_dyn_s_free_bits, device=self.device))
        kl_rep_s_loss = kl_divergence_normal(
            post_dict['s_mean'], prior_dict['s_mean'].detach(),
            post_dict['s_std'], prior_dict['s_std'].detach())
        kl_rep_s_loss = torch.max(
            kl_rep_s_loss, torch.tensor(self.config.kl_rep_s_free_bits, device=self.device))

        kl_dyn_h_loss = kl_divergence_cat(
            post_dict['h_mean'].detach(), prior_dict['h_mean'])
        kl_dyn_h_loss = torch.max(
            kl_dyn_h_loss, torch.tensor(self.config.kl_dyn_h_free_bits, device=self.device))
        kl_rep_h_loss = kl_divergence_cat(
            post_dict['h_mean'], prior_dict['h_mean'].detach())
        kl_rep_h_loss = torch.max(
            kl_rep_h_loss, torch.tensor(self.config.kl_rep_h_free_bits, device=self.device))

        kl_dyn_zp_loss = kl_divergence_normal(
            post_dict['zp_mean'].detach(), prior_dict['zp_mean'],
            post_dict['zp_std'].detach(), prior_dict['zp_std'])
        kl_dyn_zp_loss = torch.max(
            kl_dyn_zp_loss, torch.tensor(self.config.kl_dyn_zp_free_bits, device=self.device))
        kl_rep_zp_loss = kl_divergence_normal(
            post_dict['zp_mean'], prior_dict['zp_mean'].detach(),
            post_dict['zp_std'], prior_dict['zp_std'].detach())
        kl_rep_zp_loss = torch.max(
            kl_rep_zp_loss, torch.tensor(self.config.kl_rep_zp_free_bits, device=self.device))

        kl_loss = self.config.kl_dyn_s_scale * kl_dyn_s_loss + self.config.kl_rep_s_scale * kl_rep_s_loss
        kl_loss += self.config.kl_dyn_h_scale * kl_dyn_h_loss + self.config.kl_rep_h_scale * kl_rep_h_loss
        kl_loss += self.config.kl_dyn_zp_scale * kl_dyn_zp_loss + self.config.kl_rep_zp_scale * kl_rep_zp_loss

        loss = agent_rec_loss + pi_rec_loss + xp_rec_loss + reward_rec_loss + done_rec_loss + pt_loss + kl_loss
        ...
        if itr_count == self.max_epoch - 1:
            ...
            import cv2
            visualize_images_gt = agent_obs_dict['img']['data'].cpu().numpy().transpose((0, 1, 3, 4, 2))
            visualize_images_rec = agent_rec['img'].clone().detach().cpu().numpy().transpose((0, 1, 3, 4, 2))
            visualize_images_pt = pt_rec['img'].detach().cpu().numpy().transpose((0, 1, 3, 4, 2))

            for i in range(visualize_images_gt.shape[0]):
                image = np.concatenate([visualize_images_gt,
                                        visualize_images_rec,
                                        visualize_images_pt], axis=2)
                cv2.imshow(role, image[i, 0])
                cv2.waitKey(100)
                # print(i)

        return loss, agent_rec_loss, pi_rec_loss, xp_rec_loss, reward_rec_loss, pt_loss, kl_loss

    def get_prior_states(self, random_batch_length, post_dict, action, fine_tune=False):
        s_trans_mean = [[]] * random_batch_length
        s_trans_std = [[]] * random_batch_length

        h_trans_mean = [[]] * random_batch_length
        h_trans_sample = [[]] * random_batch_length
        ...
        for t in range(random_batch_length):
            if t == 0:
                if fine_tune:
                    h_trans_dist = self.hvae.h_decode_init_model_s(post_dict['s_sample'][t])
                    h_trans_mean[t] = h_trans_dist.mean
                    h_trans_sample[t] = h_trans_dist.sample()
                else:
                    h_trans_dist = self.hvae.h_decode_init_model(post_dict['s_sample'][t])
                    h_trans_mean[t] = h_trans_dist.mean
                    h_trans_sample[t] = h_trans_dist.sample()

                zp_trans_mean[t] = post_dict['zp_mean'][t]
                zp_trans_std[t] = post_dict['zp_std'][t]

                s_trans_mean[t] = post_dict['s_mean'][t]
                s_trans_std[t] = post_dict['s_std'][t]
            else:
                _prior_dict = {'s': post_dict['s_sample'][t - 1],
                               'h': post_dict['h_sample'][t - 1],
                               'zp': post_dict['zp_sample'][t - 1]}
                action_dict = {'action': action[t]}
                trans_dict = self.hvae.forward_state_prior(_prior_dict, action_dict)

                # h_trans_mean[t] = trans_dict['h_mean']
                # h_trans_sample[t] = trans_dict['h_sample']
                # h_trans_dist = self.hvae.h_decode_init_model(post_dict['s_sample'][t])
                h_trans_dist = self.hvae.h_decode_model(torch.cat([post_dict['s_sample'][t], post_dict['h_sample'][t - 1]], dim=-1))
                h_trans_mean[t] = h_trans_dist.mean
                h_trans_sample[t] = h_trans_dist.sample()

                zp_trans_mean[t] = trans_dict['zp_mean']
                zp_trans_std[t] = trans_dict['zp_std']

                s_trans_mean[t] = trans_dict['s_mean']
                s_trans_std[t] = trans_dict['s_std']

        s_trans_mean = torch.stack(s_trans_mean, dim=0)
        s_trans_std = torch.stack(s_trans_std, dim=0)

        h_trans_mean = torch.stack(h_trans_mean, dim=0)
        h_trans_sample = torch.stack(h_trans_sample, dim=0)

        zp_trans_mean = torch.stack(zp_trans_mean, dim=0)
        zp_trans_std = torch.stack(zp_trans_std, dim=0)

        return {'h_mean': h_trans_mean, 'h_sample': h_trans_sample,
                's_mean': s_trans_mean, 's_std': s_trans_std,
                'zp_mean': zp_trans_mean, 'zp_std': zp_trans_std}
